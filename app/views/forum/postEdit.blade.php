...
@section('breadcrumbs')
    ...
    array('text' => $post->title, 'url' => url('forum/'.str_replace(' ', '-', $topic->name).'/'.$post->id))
    ), 'Edit') }}
@overwrite
    ...
    <div id="add-reply" class="col-xs-12">
        <div class="row">
            ...
            <div class="col-xs-7">
                ...
                {{ Form::textarea('text', $post->text, array('id' => 'reply-textarea')) }}
                <p id="preview-reply" class="reply-content">{{ nl2br(e($post->text)) }}</p>
                <div class="row">
                    <div class="col-xs-12 col-md-4">
                        {{ Form::submit('Edit Post', array('class' => 'flat-button flat-button-small flat-button-green')) }}
                    </div>
                </div>
                {{ Form::close() }}
            </div>
